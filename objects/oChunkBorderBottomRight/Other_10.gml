...
//We're heading downright? Then everything must be pulled upleft one.
//So start from left and top.
for (_y = 0; _y < _w; _y++)
{
	for (_x = 0; _x < _w; _x++)
	{
					
		//In the special case of the topright and bottomleft cells...
		if ((_y == 0 && _x == _w - 1)|| (_y == _h - 1 && _x == 0))
		{
			...
			//Then we load whatever would be one downright of it. Currently just loads whatever.
		...
		//Now if the cell is right or bottom, it needs to be loaded.
		else if (_y == _h - 1 || _x == _w - 1)
		{
		...
		//Otherwise, we copy the cell downright of it. If this is a top row or left column chunk, we delete the old value
