/// @description Player crosses border.
...
//We're moving up. Pull everything down one, deleting the bottom row.
for (_y = 0; _y < _h; _y++)
{
	for (_x = 0; _x < _w; _x++)
	{
		...
		if (_y == _h - 1)
		{
			...
			ds_grid_set(worldGrid, _x, _y, _unloadedChunk);
		}
		...
		else 
		{
			if (_y == 0)
			{		
				_workingChunk = ds_grid_get(worldGrid, _x, _y);
				instance_destroy(_workingChunk);
			}
			_workingChunk = ds_grid_get(worldGrid, _x, _y + 1);
			ds_grid_set(worldGrid, _x, _y, _workingChunk);
		}
	}
}